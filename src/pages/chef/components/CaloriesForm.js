                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <Box sx={{ width: '80%', margin: '20px auto' }}>
                        ...
                        <Slider
                            ...
                            getAriaLabel={(index) => (index === 0 ? 'Fats' : index === 1 ? 'Carbs' : 'Proteins')}
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
                            <Typography>Proteins: {values[0].toFixed(0)}% ({calculateGrams(values[0], 4)}g)</Typography>
                            <Typography>Carbs: {(values[1] - values[0]).toFixed(0)}% ({calculateGrams(values[1] - values[0], 4)}g)</Typography>
                            <Typography>Fats: {(100 - values[1]).toFixed(0)}% ({calculateGrams(100 - values[1], 9)}g)</Typography>
                        </Box>
                    </Box>
