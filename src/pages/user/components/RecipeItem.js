...
const RecipeItem = ({ id, isFavorite, calories, time, cuisineType, dishType, mealType, image, name, recipeID }) => {
    ...
    const calorieText = isNaN(Math.floor(calories)) ? "No calories" : Math.floor(calories) + " calories";
    const timeText = time > 0 ? time + " minute(s)" : "less than a minute";
    const cuisineTags = cuisineType && cuisineType.map((cuisine) => <span key={cuisine}>{cuisine}</span>);
    const dishTags = dishType && dishType.map((dish) => <span key={dish}>{dish}</span>);
    const mealTags = mealType && mealType.map((meal) => <span key={meal}>{meal}</span>);
    ...
    const removeFromFavoriteHandler = () => {
        axios({
            ...
            .then((res) => {
                if (res.status === 200) {
                    alert(res.data.message);
                    dispatch(recipeActions.removeFromFavorites(recipeID));
                } else {
                    alert(res.error);
                }
            })
